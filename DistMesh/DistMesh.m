...
myDelaunay[ pts_] := Block[{tmp, dim, res},
...
mkcForces[ chCode_, dim_Integer] := 
 Compile[{{p, _Real, 2}, {bars, _Integer, 2}, {fScale, _Real, 0}},
  Block[{barVec, len, hBars, l0, force, forceVec},
   ...
   l0 = hBars*fScale*
     Power[(Plus @@ (len^dim))/(Plus @@ (hBars^dim)), 1/dim];
   force = Max[#, 0.] & /@ (l0 - len);
...
moveMeshPoints[ p_, bars_, cForces_, fScale_] := Module[
  {totForce, forceVec},
...
initialMeshPoints[{{x1_, x2_}, {y1_, y2_}}, h0_] := Module[
   { xRow, yColumn, tmp },
   xRow = Range[x1, x2, h0];
   yColumn = Range[ y1, y2, h0*Sqrt[3]/2 ];
   tmp = Transpose[{
      Flatten[Transpose[Table[
         If[OddQ[i], xRow, xRow + h0/2], {i, Length[yColumn ]}]]], 
...
initialMeshPoints[{{x1_, x2_}, {y1_, y2_}, {z1_, z2_}}, h0_] := Module[
   { xP, yP, zP},
   xP = Range[x1, x2, h0];
   yP = Range[y1, y2, h0];
   zP = Range[z1, z2, h0];
...
DistMesh[ nr_NumericalRegion, opts : OptionsPattern[DistMesh]] := 
 Module[
  ...
  mcmd = Region`Mesh`Utilities`ProcessMaxCellMeasure[bbox, 
    MaxCellMeasure -> mcm, "ElementType" -> etype];
  ...
  While[ True,
   ...
   If[saveBestMeshQ && mq > mqMax,
    mqMax = mq; pBest = p; tBest = t;
    ];
   
   maxf = 
    Max[deltat*
      Sqrt[Total[
         totForce[[ greaterEqualZeroPosition[d + geps] ]]^2, {2}]]/h0];
  ...
  ToElementMesh["Coordinates" -> p, 
   "MeshElements" -> {MeshElementType[dim, Last[Dimensions[t]], 1][
      t]}, FilterRules[{opts}, Options[ToElementMesh]]]
  ]
